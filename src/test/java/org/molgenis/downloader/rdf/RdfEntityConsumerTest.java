...
import static org.molgenis.downloader.rdf.RdfEntityConsumer.*;
...
public class RdfEntityConsumerTest
{
	...
	public void setupBeforeClass() throws URISyntaxException
	{
		...
		labelTag.setObjectIRI(new URI("test:label"));
		labelTag.setRelationIRI(new URI(IS_ASSOCIATED_WITH_URI));
	...
	public Iterator<Object[]> provideConversionData()
	{
		...
		IRI subject = valueFactory.createIRI("http://molgenis.org/test/1");
		IRI predicate = valueFactory.createIRI(labelTag.getObjectIRI().toString());
		...
		{ // XREF
			IRI object = valueFactory.createIRI(DEFAULT_NAME_SPACE + "ref/1");
		...
		{ // MREF
			IRI object1 = valueFactory.createIRI(DEFAULT_NAME_SPACE + "ref/1");
			IRI object2 = valueFactory.createIRI(DEFAULT_NAME_SPACE + "ref/2");
			IRI object3 = valueFactory.createIRI(DEFAULT_NAME_SPACE + "ref/3");
